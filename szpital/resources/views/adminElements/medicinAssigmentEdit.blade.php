                ...
                <div class="form-group col-md-6">
                    ...
                    <select id="medicinId" class="form-select" name="medicin_id">
                        @foreach ($medicins as $medicin)
                            <option value="{{ $medicin->id }}"
                                {{ $medicin->id == $assignmentMedicine['MEDICIN_ID'] ? 'selected' : '' }}>
                    ...
                    <select id="patientId" class="form-select" name="patient_id">
                        @foreach ($patients as $patient)
                            <option value="{{ $patient->id }}"
                                {{ $patient->id == $assignmentMedicine['PATIENT_ID'] ? 'selected' : '' }}>
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <input type="date" class="form-control" id="expirationDate" name="expiration_date"
                        value="{{ $assignmentMedicine['EXPIRATION_DATE'] }}" required>
                </div>
                <div class="form-group col-md-6">
                    <label for="availability">Dostępność</label>
                    <select id="availability" class="form-select" name="availability">
                        <option value="1" {{ $assignmentMedicine['AVAILABILITY'] == '1' ? 'selected' : '' }}>
                            Dostępny</option>
                        <option value="0" {{ $assignmentMedicine['AVAILABILITY'] == '0' ? 'selected' : '' }}>
                            Niedostępny</option>
                    </select>
                </div>
            </div>
