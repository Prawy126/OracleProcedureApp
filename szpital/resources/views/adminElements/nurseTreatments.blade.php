                ...
                <div class="form-group col-md-6">
                    ...
                    <select id="nurseId" class="form-select" name="nurse_id">
                        @foreach ($data['nurses'] as $nurse)
                            <option value="{{ $nurse->id }}">{{ $nurse->id }} {{ $nurse->name }}
                                {{ $nurse->surname }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-6">
                    ...
                    <tbody>
                        @forelse($data['treatmentNurses'] as $treatmentNurse)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('treatmentNurses.edit', $treatmentNurse->id) }}"
                                        class="btn btn-warning">Edytuj</a>
                                    <form action="{{ route('treatmentNurses.destroy', $treatmentNurse->id) }}"
