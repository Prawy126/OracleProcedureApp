    ...
    <div class="container">
        ...
        <form action="{{ route('patientsUpdate', $patient['ID']) }}" method="POST">
                    ...
                    <select class="form-control" id="inputNurseID" name="nurse_id" required>
                        ...
                        @foreach ($nurses as $nurse)
                            <option value="{{ $nurse->id }}"
                                {{ $patient['NURSE_ID'] == $nurse->id ? 'selected' : '' }}>
                ...
                <div class="col-md-4">
                    <select class="form-control" id="inputUserID" name="user_id" required>
                        ...
                        <option value="{{ $patient['USER_ID'] }}">
                            {{ $patient['USER_ID'] == $patient['USER_ID'] ? 'nie zmieniaj' : '' }}</option>
                        @foreach ($user_ids as $user)
                            <option value="{{ $user->id }}"
                                {{ $patient['USER_ID'] == $user->id ? 'selected' : '' }}>
                                {{ $user->id }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row mb-4">
                ...
                <div class="col-md-4">
                    <input type="number" class="form-control" id="inputTimeVisit" name="time_visit"
